...
├── fraud_detection/
│   ├── benchmark.py              # reproducible end-to-end benchmark command
...
Run the authoritative reusable benchmark from the repository root:

```bash
python -m fraud_detection.benchmark data/creditcard.csv
```

The command loads and validates the source CSV, creates deterministic 64/16/20
train/validation/test splits, fits preprocessing on training transactions only,
trains Logistic Regression, Isolation Forest, and CatBoost, selects each review
threshold on validation data, and prints structured validation and final test
reports to standard output. Use `--recall-target` or `--random-state` to override
the documented benchmark defaults.

Test transactions do not contribute to preprocessing state, model fitting, early
stopping, or threshold selection. Test labels are used only for the final reports.
The command does not persist a required benchmark artifact; redirect its JSON
output explicitly when a local result file is useful.

A full-data run is an optional slower manual verification because it requires the
local Kaggle dataset and CatBoost training. Fast automated checks use controlled
synthetic data and do not require `data/creditcard.csv`.

The legacy notebooks remain available for the existing exploration and analysis
workflow until their planned migration in a later issue:
